...
> Free model access inside DeepSeek Harness, with no sign-up and no API key to
> paste. The roster follows upstream, availability is measured from **your own**
> network egress, the thinking-effort control sends a real budget instead of a
> prompt hint, and a local OpenAI-compatible forward port comes included.
>
