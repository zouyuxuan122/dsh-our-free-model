...
| 速度计量 | 实测一次调用 `output=802 / reasoning=675`，reasoning 帧数为 **0**，189 ms 窗口：直接相除是 4 243 tok/s，插件选择**不报**；而流式带 reasoning 的那次 68 tok/s 照常显示 |
...
- 设置页的接口挂在应用自己的 HTTP 服务上，命名空间独立，不扩展任何共享设置面。诚实说明它的安全边界：**这些路由本身没有鉴权，也没有来源围栏**——实测不带 token 的回环请求返回 200，`Origin: http://example.com` 一样通过，`POST /forward/rotate` 裸 curl 就能改成 200。所以：本机任意进程可读写；浏览器里的跨源页面**读不到响应体**（这些路由不发 CORS 头），但**能发出改变状态的 POST**（CSRF 面）。转发端口不一样，它强制校验 Key（无 Key 请求 401），Key 用 `crypto` 生成、`timingSafeEqual` 比对、存在 `0600` 文件里；新装时 Key 字段为空，点过生成/轮换才存在。
